...
export default function BilibiliCallbackPage() {
    ...
    useEffect(() => {
        ...
        const connect = async () => {
            try {
                const res = await api.platforms.connectBilibili(code);
                if (res.success) {
                    setStatus('success');
                    setTimeout(() => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-muted/30">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-lg max-w-md w-full text-center space-y-6 border border-slate-200 dark:border-slate-800">
                {status === 'loading' && (
                    <>
                        <Loader2 className="size-12 animate-spin text-blue-400 mx-auto" />
                        <h2 className="text-xl font-bold">正在连接B站...</h2>
                        <p className="text-muted-foreground">正在获取用户信息</p>
                    </>
                )}
                ...
                {status === 'success' && (
                    <>
                        ...
                        <p className="text-muted-foreground">B站账号已成功连接到 SoloMedia</p>
