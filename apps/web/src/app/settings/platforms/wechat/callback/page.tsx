...
export default function WeChatCallbackPage() {
    ...
    useEffect(() => {
        ...
        const connect = async () => {
            try {
                const res = await api.platforms.connectWechat(code);
                if (res.success) {
                    setStatus('success');
                    setTimeout(() => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-muted/30">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-lg max-w-md w-full text-center space-y-6">
                {status === 'loading' && (
                    <>
                        <Loader2 className="size-12 animate-spin text-green-600 mx-auto" />
                        <h2 className="text-xl font-bold">正在连接微信...</h2>
                        <p className="text-muted-foreground">请稍候，正在获取您的授权信息</p>
                    </>
                )}
                ...
                {status === 'success' && (
                    <>
                        ...
                        <p className="text-muted-foreground">您的微信公众号已成功连接到 SoloMedia</p>
